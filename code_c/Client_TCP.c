...
void ParseData(char chr)
{
  ...
  //                case 0x57:
  //                    GPS[0] = (float)sData[0] / 10000000;
  //                    GPS[1] = ((float)sData[1] % 10000000) / 1e5;
  //                    GPS[2] = (float)sData[2] / 10000000;
  //                    GPS[3] = ((float)sData[3] % 10000000) / 1e5;
  //                    printf("Longitude:%ldDeg%.5fm Lattitude:%ldDeg%.5fm\r\n", GPS[0], GPS[1], GPS[2], GPS[3]);
  //                    break;
    }
    chrCnt=0;
    printf("Print Message ：%s",ReturnAcc);
...
void *servo_control()
{
  ...
  printf("servo control thread closed\r\n");
	pthread_exit(NULL);
}
